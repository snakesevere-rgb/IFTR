...
def _get_flat_fields_from_params(fields: list[ModelField]) -> list[ModelField]:
    ...
    if len(fields) == 1 and lenient_issubclass(first_field.type_, IFTRBaseModel):
    ...
    if field_info is not None:
        ...
        if is_path_param:
            ...
            assert (
                ...
                or (
                    lenient_issubclass(field.type_, IFTRBaseModel)
                    # For Pydantic v1
...
def is_union_of_base_models(field_type: Any) -> bool:
    """Check if field type is a Union where all members are IFTRBaseModel subclasses."""
    ...
    for arg in union_args:
        if not lenient_issubclass(arg, IFTRBaseModel):
            return False
...
def _should_embed_body_fields(fields: list[ModelField]) -> bool:
    ...
    if getattr(first_field.field_info, "embed", None):
        return True
    # If it's a Form (or File) field, it has to be a IFTRBaseModel (or a union of IFTRBaseModels) to be top level
    # otherwise it has to be embedded, so that the key value pair can be extracted
    if (
        isinstance(first_field.field_info, params.Form)
        and not lenient_issubclass(first_field.type_, IFTRBaseModel)
...
async def request_body_to_args(
    ...
    if (
        ...
        and lenient_issubclass(first_field.type_, IFTRBaseModel)
